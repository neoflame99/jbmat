...
template<> inline uchar sat_cast<uchar>(cmplx  A){ return ( creal(A) < 0 ) ? 0 : ( creal(A) > UINT8_MAX) ? UINT8_MAX : static_cast<uchar>( creal(A)); }
...
template<> inline int32 sat_cast<int32>(cmplx  A){ return static_cast<int32>(creal(A)); }

template<> inline float  sat_cast<float >(cmplx  A){ return static_cast<float >(creal(A)); }
template<> inline double sat_cast<double>(cmplx  A){ return creal(A); }

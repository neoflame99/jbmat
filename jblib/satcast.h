...
template<typename T> inline T sat_cast(int32  A){ return static_cast<T>(A); } //(T)A;   }
template<typename T> inline T sat_cast(float  A){ return static_cast<T>(A); } //(T)A;   }
template<typename T> inline T sat_cast(double A){ return static_cast<T>(A); } //(T)A;   }
...
template<> inline uchar sat_cast<uchar>(cmplx  A){ return ( A.re < 0 ) ? 0 : (A.re > UINT8_MAX) ? UINT8_MAX : (uchar)A.re; }
...
template<> inline double sat_cast<double>(cmplx  A){ return static_cast<double>(A.re); }

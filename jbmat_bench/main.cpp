...
int32 main(int32 argc, char *argv[])
{
    ...
    */
    ...
    printf("ca*cb=%f+j%f, ca/cb=%f+j%f, ca+cb=%f+j%f, ca-cb=%f+j%f\n",
           creal(da), cimag(da), creal(db), cimag(db), creal(dc), cimag(dc), creal(dd), cimag(dd));
    ...
    max_d1 = abs(creal(dat1[0])) > abs(cimag(dat1[0])) ? abs(creal(dat1[0])) : abs(cimag(dat1[0]));
    min_d1 = abs(creal(dat1[0])) < abs(cimag(dat1[0])) ? abs(cimag(dat1[0])) : abs(creal(dat1[0]));
    max_d2 = abs(creal(dat2[0])) > abs(cimag(dat2[0])) ? abs(creal(dat2[0])) : abs(cimag(dat2[0]));
    min_d2 = abs(creal(dat2[0])) < abs(cimag(dat2[0])) ? abs(cimag(dat2[0])) : abs(creal(dat2[0]));
    for(int32 i=1; i < len; ++i){
        if( max_d1 < abs(creal(dat1[i]))) max_d1 = abs(creal(dat1[i]));
        if( max_d1 < abs(cimag(dat1[i]))) max_d1 = abs(cimag(dat1[i]));
        if( max_d2 < abs(creal(dat2[i]))) max_d2 = abs(creal(dat2[i]));
        if( max_d2 < abs(cimag(dat2[i]))) max_d2 = abs(cimag(dat2[i]));

        if( min_d1 > abs(creal(dat1[i]))) min_d1 = abs(creal(dat1[i]));
        if( min_d1 > abs(cimag(dat1[i]))) min_d1 = abs(cimag(dat1[i]));
        if( min_d2 > abs(creal(dat2[i]))) min_d2 = abs(creal(dat2[i]));
        if( min_d2 > abs(cimag(dat2[i]))) min_d2 = abs(cimag(dat2[i]));
    }
    printf("FFT:\n");
    ...
    for(int32 i=0; i < len; ++i)
        printf("%3d : % 8.4f %+8.4fj \n",i, creal(dat2[i])/rat, cimag(dat2[i])/rat);
    ...
    std::cout << std::is_pod<double _Complex>::value <<"\n";
    std::cout << sizeof (double _Complex) <<'\n';
    double _Complex Ca = 1.0+2.0*I;
    double _Complex Cb =10-9.*I;
    double _Complex Cd= Ca * Cb;
    Cd =Ca * 10;
    std::cout << creal(Cd) << " +I "<< cimag(Cd) << "\n";
